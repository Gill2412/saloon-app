...
const AppointmentRouters =require('./src/routers/Appointments')
...
router.use(cors())
router.use(express.json()); //We include support for JSON that is coming from the clients

router.use(AppointmentRouters)
...
//     var xml = fs.readFileSync('Salon.xml', 'utf8'); //We are reading in the XML file
//     var xsl = fs.readFileSync('Salon.xsl', 'utf8'); //We are reading in the XSL file
...
//     xmlFileToJs('Salon.xml', function(err, result) {
//       if (err) throw (err);
...
//       result.Appointments.Days[obj.sec_n].Details.push({'Client': obj.Client, 'Time': obj.Time}); //If your XML elements are differet, this is where you have to change to your own element names
//       //Converting back to our original XML file from JSON
//       jsToXmlFile('Salon.xml', result, function(err) {
...
//       delete result.Appointments.Days[obj.Days].Details[obj.Details];
